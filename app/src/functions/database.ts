...
class RemedeDatabase {
    ...
    async getAutocomplete(query: string) {
        const statement = `SELECT word FROM dictionary WHERE indexed LIKE '${query}%' ORDER BY word ASC LIMIT 5`
        return await this.query(statement)
    }

    async search(query: string, page = 0) {
        const statement = `SELECT word FROM dictionary WHERE indexed LIKE '${query}%' ORDER BY word ASC LIMIT 50 OFFSET ${page * 50}`
        return await this.query(statement)
    }
    ...
    async getWordsWithPhoneme(phoneme: string) {
        const statement = `SELECT document FROM dictionary WHERE phoneme = '${phoneme}'`
        const response = await this.rawQuery(statement) as string[][]
        return response.flatMap(document => JSON.parse(document[0]))
    }
