...
A table `wordlist` is also included to make the search faster:
- A field `word` which contains the word
- A field `indexed` which contains the word, but without accents, without `-` and `'` and lower cased
